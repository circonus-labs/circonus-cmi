...
list.prototype.ec2_updates = function(o, alt_cb) {
    ...
    var node_count = 0;
    for(var n in results.circonus.nodes)
        if(results.circonus.nodes.hasOwnProperty(n)) node_count++;
            ...
            nlist.forEach(function(node) {
                node.forEach(function (check_bundle) {
                    ...
                    tags.forEach(function(tag) {
                        var etagsLen = etags.filter(function(a){
                            return (a===tag);
                        });
                        if(etagsLen === 0)
                            newtags++;
...
list.prototype.chef_updates = function(chefList){
    ...
    for(var cmiID in chefChecks){
        var chefCheck = chefChecks[cmiID];
        if(existingChecks[cmiID]){
            // TODO: Add some comparison in here to save on unnecessary api
            // calls
            chefCheck._cid = existingChecks[cmiID]._cid;
            updateChecks.push((function(payload) {
                return function(cb) {
                    self.circonus.put(payload._cid, payload,
                    function(code, error, body) {
                        if(code == 200) cb(null, payload._cid);
